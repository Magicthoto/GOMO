				      ...
				      PlayerColor color, int diff)
{
  ...
  if (point.direction(oppositeDir).length == 4 && point.isTaken() == color) {
  ...
  if (_winner == PlayerColor::NONE) {
    ...
    if (! out_of_bounds && point.isTaken() == color) {
      isOpen = ! _propagateInfo(next, dir, color, diff);
  ...
  return (point.isTaken() != color && point.isTaken() != PlayerColor::NONE);
}
					     ...
					     PlayerColor color, int diff)
{
  ...
  next = Traveller::travel(index, dir, out_of_bounds);
  if (! out_of_bounds && _points[index].isTaken() == color) {
